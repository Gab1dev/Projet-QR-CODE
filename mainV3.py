...
def patternFixe(qr_code : list) -> None:
    ...
    placePatternFinder(29-7, 0)
    placePatternFinder(0, 29-7)
    placePatternTiming()
    ...
    while col > 0:
        while 0 <= row < size:
            for i in [0,-1]:
                colonne = col + i
                ligne = row
                if 0 <= colonne < 29 and 0 <= ligne < 29 and qr_code[ligne][colonne] is None:
                    if (ligne)%2 == 0:
...
def genereCorrectionErreur(data_bits : str) -> str:
    ...
    ec_bits = ''.join(f'{byte:08b}' for byte in data_encode[-15:])
    print(len(ec_bits))
    return ec_bits
...
QR_Code = [[None for i in range(29)]for i in range(29)]
patternFixe(QR_Code)
placeBits(QR_Code,encodement("text"))
placeBits(QR_Code,genereCorrectionErreur(encodement("text")))
dessineQR(QR_Code)
